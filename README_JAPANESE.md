...
### 式の内容が変化する場合は速度が落ちるが、複数インスタンスを使い分けると回避可能
...
なお、より明示的に、Exevalator 内部のキャッシュを使って、前回 eval 時と同じ式を計算する「 reeval( ) 」というメソッドも用意されています。使うと Exevalator 内部で式の同一性の判定処理をスキップできるため、原理的には少し速くなる事が見込めます。
